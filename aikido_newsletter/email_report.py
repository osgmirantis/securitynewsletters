...
def render_html(report: dict, img_src: dict[str, str], workspace: str = "") -> str:
    ...
    # Oldest open findings (table) + most-repeated vuln types (chips)
    oldest_rows = ""
    for c in (report.get("oldest_open") or [])[:6]:
        od = (f'<span style="background:#fdeceb;color:{SEV["critical"]};font-size:9px;'
              f'padding:1px 5px;border-radius:4px;margin-left:6px;">OVERDUE</span>') if c["overdue"] else ""
        oldest_rows += (
            f'<tr><td align="right" style="padding:7px 12px;border-bottom:1px solid {LINE};'
            f'font-family:Consolas,Menlo,monospace;font-weight:700;color:{SEV["critical"]};">{c["age_days"]}d</td>'
            f'<td style="padding:7px 12px;border-bottom:1px solid {LINE};">{c["rule"]}{od}</td>'
            f'<td style="padding:7px 12px;border-bottom:1px solid {LINE};color:{SEV.get(c["severity"], INK)};'
            f'text-transform:uppercase;font-size:11px;">{c["severity"]}</td>'
            f'<td style="padding:7px 12px;border-bottom:1px solid {LINE};color:{MUTED};">{c["product"]} · {c["repo"] or "—"}</td></tr>')
    repeated = "".join(
        f'<span style="display:inline-block;background:#f1f3f6;border:1px solid {LINE};'
        f'border-radius:6px;padding:4px 9px;margin:0 6px 6px 0;font-size:12px;color:{INK};">'
  ...
  <table width="680" cellpadding="0" cellspacing="0" role="presentation"
    ...
    <tr><td style="padding:26px 28px;font-family:Helvetica,Arial,sans-serif;color:{INK};">
      ...
      <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
        ...
        {section_title("⏳ Oldest open findings")}
        <tr><td style="padding-top:8px;">
          <table width="100%" cellpadding="0" cellspacing="0" role="presentation"
            style="border:1px solid {LINE};border-radius:8px;overflow:hidden;font-size:13px;color:{INK};">
            <tr style="background:#f4f5f7;font-size:11px;letter-spacing:.5px;text-transform:uppercase;color:{MUTED};">
              <td align="right" style="padding:8px 12px;">Age</td><td style="padding:8px 12px;">Finding</td>
              <td style="padding:8px 12px;">Severity</td><td style="padding:8px 12px;">Product · asset</td></tr>
            {oldest_rows}
          </table></td></tr>
        {section_title("🔁 Most repeated vulnerability types")}
        <tr><td style="padding-top:12px;">{repeated}</td></tr>
...
def render_text(report: dict, workspace: str = "") -> str:
    ...
    if report.get("top_vuln_types"):
        L += ["", "MOST REPEATED: " + ", ".join(f"{t} ×{n}" for t, n in report["top_vuln_types"][:6])]
    if report.get("mttr_by_type"):
        L += ["", "SLOWEST TO REMEDIATE (median MTTR by type)"]
        for d in report["mttr_by_type"][:6]:
            L.append(f"  {d['type'][:34]:<36}{d['median']:>5}d  (n={d['n']})")
    if report.get("oldest_open"):
        L += ["", "OLDEST OPEN FINDINGS"]
        for c in report["oldest_open"][:6]:
            L.append(f"  {str(c['age_days']) + 'd':>6}  [{c['severity']}] {c['rule']} — "
                     f"{c['product']} · {c['repo'] or '-'}{' OVERDUE' if c['overdue'] else ''}")
